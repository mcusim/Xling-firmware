 ...
 *
 *
 * Low-level driver for SH1106-based displays connected by 4-wire SPI
 * interface.
 */
#include <stdint.h>
#include <string.h>
...
struct MSIM_SH1106 {
	...
	volatile uint16_t sent_len;	/* Number of the buffer bytes to send */
	volatile sh1106_con_state state;/* Display's connection state */
	volatile sh1106_dtype dat_type; /* Buffer data type (CMD or GRAM) */
	volatile uint8_t init;		/* Display initialized flag */
	volatile uint8_t buf[BUFSZ];	/* Driver's buffer */
};

static int	append_byte(MSIM_SH1106 *, sh1106_dtype, const uint8_t);
static int	append_bytes(MSIM_SH1106 *, sh1106_dtype, const uint8_t *, size_t);
static int	append_bytes_PF(MSIM_SH1106 *, sh1106_dtype, const uint8_t *, size_t);

static uint8_t drv_init = 0;			/* A "driver init" flag */
static struct MSIM_SH1106 *cdev;		/* Current DCB (for ISR) */
static struct MSIM_SH1106 devices[DNUM];	/* Pool of the DCBs */

/* Initializes the driver (3-wire SPI). */
int
MSIM_SH1106__drvStart(const MSIM_SH1106DriverConf *conf)
{
	...
	/* Enable SPI, its interrupt, master mode, and mode 3. */
	...
	/* There is no current DCB available yet. */
	cdev = NULL;
...
/* Performs a de-initialization of the driver (3-wire SPI). */
int
...
{
	append_byte(dev, DC_CMD, MSN(CMD_COLHADDR) | LSN(col>>8));
...
/* Appends a command to the driver's buffer to set the display's contrast. */
int
...
static int
append_byte(MSIM_SH1106 *dev, sh1106_dtype dc, const uint8_t byte)
{
	int rc = MSIM_SH1106_RC_OK;
	...
	do {
		...
		/* There should be a place in the buffer for 2 bytes */
...
static int
append_bytes(MSIM_SH1106 *dev, sh1106_dtype dc, const uint8_t *data, size_t len)
{
	int rc = MSIM_SH1106_RC_OK;
...
static int
append_bytes_PF(MSIM_SH1106 *dev, sh1106_dtype dc, const uint8_t *d, size_t len)
{
